...
if (len(sys.argv) != 4):
    print "\nUsage: python acc-plotter.py [data.csv] [num-channels] [graph-freq]\n"
    print "[data.csv] is the data CSV file which should be plotted\n"
    print "[num-channels] is the number of data channels which should be plotted"
    print "The data channels will be plotted in the consecutive order found in the file starting with the second column, where the first column is the x axis.\n"
    ...
    print "Any non-zero value must be lower than the frequency of data in the given file.\n"
    sys.exit()
#
#[start-time] is the start time of the data to be plotted.  This can be given either in the format YYYY/MM/DD@HH:MM:SS or in seconds after midnight.  A value of 0 will plot the entire time range of the given data file.  
#
...
if numchannels == 1:
    ...
    plot.stdin.write("plot '%s' every %d::%d using ($1+%d):2 with linespoints\n" % (filename, every, md, epoch))
    plot.stdin.write("quit\n")
else:
    # First line
    plot.stdin.write("plot '%s' every %d::%d using ($1+%d):2 with linespoints," % (filename, every, md, epoch))
    # Loop through the middle portion of the requested channels
    for i in range(3, numchannels + 1):
        plot.stdin.write("'%s' every %d::%d using ($1+%d):%d with linespoints," % (filename, every, md, epoch, i))
    # Final line
    plot.stdin.write("'%s' every %d::%d using ($1+%d):%d with linespoints\n" % (filename, every, every, md, epoch, numchannels + 1))
    plot.stdin.write("quit\n")
